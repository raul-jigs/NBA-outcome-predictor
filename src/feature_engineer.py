...
def collect_all_traditional_stats():
    playerDF= pd.DataFrame(columns=['Player', 'Team', 'Match Up', 'Game Date', 'W/L', 'MIN', 'PTS', 'FG%', '3P%', 'FT%', 'OREB', 'DREB', 'AST', 'STL', 'BLK', 'TOV', 'PF', '+/-'])
    astypeDict= {'Player':str, 'Team':str, 'Match Up':str, 'Game Date':str, 'W/L':str, 'MIN':float, 'PTS':float, 'FG%':float, '3P%':float, 'FT%':float, 'OREB':float, 'DREB':float, 'AST':float, 'STL':float, 'BLK':float, 'TOV':float, 'PF':float, '+/-':float}
    ...
    for year in tqdm(range(8, 22)):
        ...
        if "Season" in df:
            ...
            df=df.drop(columns=["FGM", "FGA", "3PM", "3PA", "FTM", "FTA", "REB"])
    ...
    pd.DataFrame(playerDF).to_csv("/Users/raulalcantar/Downloads/NBA_Game_Predictor-main/data/NBA_Advanced(NOTREALLY)_Season_Average_Stats.csv", header=['Player', 'Team', 'Match Up', 'Game Date', 'W/L', 'PTS', 'FG%', '3P%', 'FT%', 'OREB', 'DREB', 'REB', 'AST', 'STL', 'BLK', 'TOV', 'PF', '+/-'], index=None)
...
def collect_all_advanced_stats():
    ...
    playerDF = pd.concat(frames)
    pd.DataFrame(playerDF).to_csv("/Users/raulalcantar/Downloads/NBA_Game_Predictor-main/data/NBA_Advanced_Season_Average_Stats.csv", header=['Player', 'Team', 'Match Up', 'Game Date', 'W/L', 'MIN', 'OFF_RATING','DEF_RATING','NET_RATING','AST%','AST_TO','AST_RATIO','OREB%','DREB%','REB%','TM_TOV%','EFG%','TS%','USG%','PACE'], index=None)
