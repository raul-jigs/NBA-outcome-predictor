...
playerDF= pd.DataFrame(columns=['Player', 'Team', 'Match Up', 'Game Date', 'W/L', 'PTS', 'FG%', '3P%', 'FT%', 'OREB', 'DREB', 'REB', 'AST', 'STL', 'BLK', 'TOV', 'PF', '+/-'])
astypeDict= {'Player':str, 'Team':str, 'Match Up':str, 'Game Date':str, 'MIN':float, 'PTS':float, 'FG%':float, '3P%':float, 'FT%':float, 'OREB':float, 'DREB':float, 'AST':float, 'STL':float, 'BLK':float, 'TOV':float, 'PF':float, '+/-':float}
groupingDict= {'Player':'first', 'Team': 'first', 'Match Up':'first', 'Game Date':'first', 'MIN':'sum', 'PTS':'sum', 'FG%':'sum', '3P%':'sum', 'FT%':'sum', 'OREB':'sum', 'DREB':'sum', 'AST':'sum', 'STL':'sum', 'BLK':'sum', 'TOV':'sum', 'PF':'sum', '+/-':'sum'}
frames=[]
...
pd.DataFrame(playerDF).to_csv("/Users/raulalcantar/Downloads/NBA_Game_Predictor-main/data/NBA_Average_Stats.csv", header=['Player', 'Team', 'Match Up', 'Game Date', 'W/L', 'PTS', 'FG%', '3P%', 'FT%', 'OREB', 'DREB', 'REB', 'AST', 'STL', 'BLK', 'TOV', 'PF', '+/-'], index=None)
